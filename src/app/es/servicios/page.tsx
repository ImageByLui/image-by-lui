...
export const metadata: Metadata = { ...buildPageMetadata(content.seo, "es", "services"), alternates: { canonical: "/es/servicios", languages: { en: "/services", es: "/es/servicios", "x-default": "/services" } }, openGraph: { title: "Consultoría de Imagen y Estilismo de Ocasión en Miami", description: "Estrategia de imagen profesional basada en colorimetría.", images: [{ url: "/og/services-overview.jpg", width: 1200, height: 630 }], locale: "es_US", type: "website" } };
...
export default function ServiciosPage() {
  return (<>
    ...
    <div className="bg-warm-ivory py-3 px-5 text-center"><p className="font-heading font-light italic text-[13px] text-warm-taupe">Confiada por profesionales en todo el condado de Miami-Dade</p></div>
    <section className="bg-warm-ivory pt-4 px-3.5 pb-3.5" aria-label="Nuestros servicios">
      <p className="font-body font-semibold text-[13px] text-espresso text-center tracking-[1px] uppercase mb-1">{content.selector.heading}</p>
      <p className="font-body text-[13px] text-warm-taupe text-center leading-[1.5] mb-[22px]">{content.selector.subheading}</p>
      {content.selector.cards.map((card: ServiceSelectorCardData) => <ServiceSelectorCard key={card.name} {...card} />)}
    </section>
    <ServicesTestimonialBlock quote={content.testimonial.quote} name={content.testimonial.name} initials={content.testimonial.initials} title={content.testimonial.title} />
    <BottomCTASection>
      ...
      <a href={content.bottomCta.primaryCta.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2.5 bg-[#25D366] text-white font-body font-semibold text-[14px] tracking-[1px] uppercase px-8 py-4 no-underline hover:no-underline transition-colors duration-150 active:bg-[#1DA851] focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2">
      ...
      <p className="font-body text-[13px] text-warm-ivory/60 mt-3">{content.bottomCta.secondaryText}{" "}<a href={content.bottomCta.secondaryLinkHref} target="_blank" rel="noopener noreferrer" className="text-gold font-semibold underline hover:text-gold">{content.bottomCta.secondaryLinkText}</a></p>
    </BottomCTASection>
