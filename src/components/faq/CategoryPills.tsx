      ...
      {categories.map((cat) => {
        ...
        return (
          <button key={cat.id} onClick={() => onSelect(cat.id)} aria-pressed={isActive}
            className={`whitespace-nowrap shrink-0 font-body font-semibold text-[13px] tracking-[0.3px] px-4 py-2.5 select-none transition-all duration-150 active:scale-[0.97] focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 ${isActive ? "bg-terracotta text-white border border-terracotta" : "bg-transparent text-warm-taupe border border-champagne"}`}>
