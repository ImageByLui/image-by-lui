  ...
  return (
    <>
      <section className="bg-warm-ivory pt-8 px-5 pb-6" aria-label="Hero">
        <p className="font-body font-semibold text-[11px] tracking-[2px] uppercase text-terracotta mb-3">{content.hero.label}</p>
        <h1 className="font-heading text-[32px] font-semibold text-espresso leading-[1.18]">{content.hero.headline}</h1>
        <div className="w-12 h-[2px] bg-gold my-3.5" aria-hidden="true" />
        <p className="text-[17px] text-warm-grey leading-[1.6]">{content.hero.description}</p>
      </section>
      ...
      <BottomCTASection>
        ...
        <p className="font-body text-[15px] text-warm-ivory/60 leading-[1.6] mb-[22px]">{content.bottomCta.description}</p>
        <div className="flex gap-2.5">
          {/* V2: WhatsApp button GREEN */}
          <a href={content.bottomCta.whatsappCta.href} target="_blank" rel="noopener noreferrer"
            className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] text-white font-body font-semibold text-[12px] tracking-[0.8px] uppercase py-[15px] px-2.5 no-underline hover:no-underline transition-colors duration-150 active:bg-[#1DA851] focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2">
