    ...
    <section className="bg-soft-stone py-5 px-3.5" aria-label="Free session">
      <div className="bg-white border border-champagne overflow-hidden">
        ...
        <div className="pt-4 px-5 pb-5">
          ...
          <h2 className="font-heading font-medium text-[24px] text-espresso leading-[1.15] mb-1.5">{title}</h2>
          <div className="w-8 h-[2px] bg-gold my-3" aria-hidden="true" />
          <p className="font-body text-[15px] text-warm-grey leading-[1.6] mb-3.5">{description}</p>
          <div className="mb-4">
          ...
          <a href={cta.href} target={cta.external ? "_blank" : undefined} rel={cta.external ? "noopener noreferrer" : undefined}
            className="block text-center font-body font-semibold text-[14px] tracking-[1.5px] uppercase py-4 bg-terracotta text-white no-underline hover:no-underline hover:bg-terracotta-dark transition-colors duration-150 focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2">
            {cta.text}
          </a>
          <p className="font-body text-[13px] text-warm-taupe text-center mt-2">{footnote}</p>
        </div>
      </div>
    </section>
  );
