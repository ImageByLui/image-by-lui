    ...
    <section className="bg-soft-stone py-7 px-5" aria-label="Pricing">
      <div className="bg-white border border-gold py-6 px-5 text-center">
        <div className="flex items-baseline justify-center gap-2.5 mb-1.5">
          <span className="font-heading font-medium text-price-lg text-terracotta">{price}</span>
          <span className="font-body text-nav text-warm-taupe line-through">{priceStrikethrough}</span>
        </div>
        <p className="font-body font-semibold text-tag tracking-[1.2px] uppercase text-gold mb-[18px]">{priceNote}</p>
          ...
          {summary.map((col, i) => (
            <div key={i} className="flex items-start flex-1">
              ...
              <div className="flex-1 text-center px-1.5">
                <p className="font-heading font-medium text-card-title text-espresso leading-tight">{col.value}</p>
                <p className="font-body text-caption text-warm-taupe">{col.label}</p>
              </div>
            </div>
          ))}
        </div>
