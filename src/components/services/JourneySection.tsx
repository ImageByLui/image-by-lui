...
interface JourneySectionProps {
  ...
  /** Content rendered inside the last accordion card */
  lastStepFooter?: ReactNode;
}

export default function JourneySection({ label, subheading, components, lastStepFooter }: JourneySectionProps) {
    ...
    <section className="bg-warm-ivory py-7 px-3.5" aria-label="Your journey">
      ...
      {components.map((comp, i) => (
        <JourneyComponent
          ...
          isFirst={i === 0}
          footer={i === components.length - 1 ? lastStepFooter : undefined}
        />
      ))}
    </section>
  );
