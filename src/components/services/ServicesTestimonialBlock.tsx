...
export default function ServicesTestimonialBlock({
  ...
  return (
    <section className={`${BG_MAP[bgColor]} py-8 px-5 lg:py-0 lg:px-0 lg:bg-transparent`} aria-label="Client testimonial">
      <ScrollReveal>
        {/* Mobile layout */}
        <div className="flex gap-3.5 items-start lg:hidden">
          ...
          <div>
            ...
            {handle && <p className="font-body text-caption text-warm-taupe">{handle}</p>}
          </div>
        </div>

        {/* Desktop layout — grid with attribution sidebar */}
        <div className="hidden lg:block max-w-[1200px] mx-auto px-12 py-12">
          <div className="grid grid-cols-[1fr_auto] gap-12 items-center bg-soft-stone p-10 pl-12 border-l-[3px] border-l-gold">
            {/* Quote */}
