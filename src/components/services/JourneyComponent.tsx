...
export default function JourneyComponent({
  ...
  const summaryContent = (
    <div className="pt-5 px-5 pb-[18px] flex items-start gap-3.5">
      <span className="font-heading font-medium text-step-num text-terracotta leading-none shrink-0">{stepNumber}</span>
      <div className="flex-1 min-w-0">
        <p className="font-body font-semibold text-nav text-espresso mb-[3px]">{name}</p>
        <p className="font-body text-caption text-warm-taupe">{duration} · {subtitle}</p>
      </div>
  ...
  return (
    <AccordionCard summary={summaryContent} defaultOpen={defaultOpen} borderClasses={borderClasses} className="mb-3 last:mb-0">
      <div className="px-5 pb-[22px]">
        ...
        <p className="font-body text-card text-warm-grey leading-[1.6] mb-4">{description}</p>
        ...
        <p className="font-heading font-light italic text-inclusion text-gold mt-3.5">{deliverableLine}</p>
      </div>
    </AccordionCard>
  );
}
